...
class CP:
    '''
        ...
        cl: Cycle count
    '''
    def __init__(self, ic, ia, eh, el, tc, ta, pn, si, cl, folder, fileName, header, path_lib, **kwargs):
        self.fileName = fileName
        self.folder = folder
        self.text = ''

        self.validate(eh, el, si, cl, tc, ta)
        ...
        self.text = self.head + self.body + '\nrun\nsave:' + self.fileName + '\ntsave:' + self.fileName + '\n forcequit: yesiamsure\n'

    def validate(self, eh, el, si, cl, tc, ta):
        info = Info()
        info.limits(eh, info.E_min, info.E_max, 'eh', 'V')
        info.limits(el, info.E_min, info.E_max, 'el', 'V')
        if si <= 0:
            raise Exception('si (sample interval) must be greater than 0')
        if cl < 1:
            raise Exception('cl (cycle count) must be at least 1')
        if tc < 0 or ta < 0:
            raise Exception('tc and ta must be non-negative')
        print('All the parameters are valid')
